    ...
    return (
        <div className="min-h-screen bg-gray-50/50 font-sans text-slate-900 selection:bg-primary/20 selection:text-primary">
            ...
            <main className="space-y-24 pb-32 md:pb-24">
                ...
                {/* Fallback: Show default content when no publicContent is configured */}
                {(!organizer?.publicContent?.length) && (
                    <>
                        ...
                        <section className="relative py-20 overflow-hidden px-4 text-center space-y-8">
                            ...
                            <div className="max-w-4xl mx-auto">
                                ...
                                <h1 className="text-3xl sm:text-5xl md:text-7xl font-bold tracking-tight text-gray-900 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100 mb-6">
                                    Support <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary/60">{orgProfile.orgName || 'Our Mission'}</span>
                                </h1>
                                <p className="text-base sm:text-xl md:text-2xl text-gray-600 max-w-2xl mx-auto leading-relaxed animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
                                    {orgProfile.description || 'Connect your brand with our community. Choose a package below to make an impact today.'}
                                </p>
                            </div>
                        </section>

                        {/* Default Packages Grid */}
                        <div id="packages-section" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            ...
                            {packages.length === 0 ? (
                                <div className="text-center py-12">
                                    <p className="text-gray-500">No sponsorship packages are currently available.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {packages.map(pkg => (
                                        ...
                                        >
                                            {pkg.imageUrl && (
                                                <div className="w-full aspect-[16/9] rounded-2xl overflow-hidden mb-6 -mt-2">
                                            ...
                                            <button
                                                ...
                                                disabled={addingId === (pkg.id || pkg._id)}
                                            >
                                                {addingId === (pkg.id || pkg._id) ? (
                                                    <Loader2 className="w-4 h-4 animate-spin" />
                                                ) : (
                                                    <>
                                                        {cart.find(i => i.id === (pkg.id || pkg._id)) ? 'In Cart' : 'Select Package'}
                                                        {!cart.find(i => i.id === (pkg.id || pkg._id)) && <ArrowRight className="w-4 h-4" />}
                                                    </>
                                                )}
                                            </button>
                                        </div>
            ...
            <div className={`fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] flex items-center gap-4 md:contents ${cart.length > 0 ? 'justify-between' : 'justify-center'}`}>
